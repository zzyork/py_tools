...
def sentence():
    ...
    # sentence = response
    # name = json.loads(response)["origin"]
    # author = json.loads(response)["author"]
    # category = json.loads(response)["category"]
    # print (sentence)
    # return [sentence,name,author,category]
    return sentence
...
def img():
    ...
    x = 50
    y = 50
    font = ImageFont.truetype("C:\\Windows\\Fonts\\simkai.ttf",40)
    ...
    for k,s2 in enumerate(strs):
        ...
        draw.text((x+right, y+down),s2,(255,255,255),font=font)
